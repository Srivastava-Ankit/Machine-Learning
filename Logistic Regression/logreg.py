...
class LogReg:
    ...
    def sgd_update(self, x_i, y):
        ...
        dot_pi = np.dot(self.w, x_i)
        # Calculate the exponential
        pi_i = sigmoid(dot_pi)
        # Update the w
        for ii in range(0, len(self.w)):
            self.w[ii] = self.w[ii] + self.eta * (y - pi_i) * x_i[ii]
...
if __name__ == "__main__":
    ...
    for epoch in range(args.passes):
        ...
        for ii in data.train_x:
            iteration += 1
            lr.sgd_update(ii, iteration)

            train_lp, train_acc = lr.progress(data.train_x, data.train_y)
            test_lp,test_acc = lr.progress(data.test_x, data.test_y)
            print("Update %i\tTP %f\tHP %f\tTrain Data Acc %f\tTest Data Acc %f" %
                  (iteration, train_lp, test_lp, train_acc, test_acc))