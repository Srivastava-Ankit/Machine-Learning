...
class Knearest:
    ...
    def classify(self, example):
        ...
        _, point_set = self._kdtree.query(numpy.array(example).reshape(1, -1), self._k)
        return self.majority(point_set.flatten())
    ...
    def confusion_matrix(self, test_x, test_y, debug=False):
        ...
        d = defaultdict(dict)
        data_index = 0
        for mm, nn in zip(test_x, test_y):
            try:
                d[nn][self.classify(mm)] += 1
            except KeyError:
                d[nn][self.classify(mm)] = 1
            data_index += 1
